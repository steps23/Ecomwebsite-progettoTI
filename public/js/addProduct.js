...
discountPercentage.addEventListener('input', () => {
    if(discountPercentage.value > 100){
        ...
        let discount = (actualPrice.value * discountPercentage.value / 100).toFixed(0);
        sellPrice.value = actualPrice.value - discount;
    }
})

sellPrice.addEventListener('input', () => {
    let discount = ((sellPrice.value / actualPrice.value) * 100).toFixed(0);
...
uploadImages.forEach((fileupload, index) => {
    fileupload.addEventListener('change', () => {
        ...
        if(file.type.includes('image')){
            ...
            fetch('/s3url').then(res => res.json()).then(url => {
                fetch(url,{
                    method: 'PUT',
...
const productData = () => {
    ...
    tagArr.forEach((item, i) => tagArr[i] = tagArr[i].trim().toLowerCase()); //.toLowerCase() for a better search 
    return data = {
...
addProductBtn.addEventListener('click',() => {
    ...
    if(validateForm()){  //validateForm return true or false while doing validation
        ...
        if(!data.id){
            data.id = data.name.toLowerCase()+'-'+Math.floor(Math.random() * 5000);
        }
        if(productId){
            data.id = productId;
        }
