...
const createSmallCards = (data) => {
    ...
    <img src="${data.image}" class="sm-product-img" alt="">
        <div class="sm-text">
            ...
            <p class="sm-des">Size:${data.size}</p>
        </div>
...
const setProducts = (name) => {
    ...
    for(let i= 0; i < data.length; i ++){
            ...
            if(name=='cart'){
                    totalBill += Number((data[i].sellPrice*data[i].item).toFixed(2));
            }
            updateBill();
        }
    }
...
const setupEvents = (name) => {
    ...
    counts.forEach((item, i) => {
        ...
        counterPlus[i].addEventListener( 'click' , () => {
            let data = JSON.parse(localStorage.getItem(name));
            if (count_stock(data,product[i])+1 <= product[i].stock){
                item.innerHTML++;
                totalBill += cost;
                price[i].innerHTML = `€${item.innerHTML * cost}`;
                if(name == 'cart' ){ 
                ...
                showAlert('Sorry, stock limit reached');
            }
        })
