...
let serviceAccount = require("./ecom-website-70a16-firebase-adminsdk-e09no-9b55ba0978");
...
const bucketName = "ecom-website-progetto-amoretti";
...
app.post('/signup', (req, res) => {
    ...
    if(name.length < 3){
        ...
        return res.json({'alert': 'enter your eamil'});
...
app.post('/add-product', (req, res) => {
    ...
    //validation
    //console.log(name, shortDes, des, images, sizes, actualPrice, discount, sellPrice, stock, tags, tac, email);
    ...
    if(!draft){
        ...
        else if(!sizes.length){ // size array7
...
//place an order and sand email
app.post('/order' , (req, res) => {
    const {order, email, add } = req.body;

    let data=JSON.parse(order);
    //console.log(data);
    let start = '<h2 class="product-category">Your order:</h2> <ul> ';
    let middle = ''; // this will contain the products ordered
    let end = '</ul>';
    for(let i= 0; i < data.length; i++) {
        middle +=  `<li> ${data[i].item} ${data[i].name} taglia ${data[i].size} </li>`
    }
    let list_html= start+middle+end;
    ...
    const mailOption ={
        from: 'progettositoamoretti@gmail.com' ,
        to: email,
        ...
        <body>
            ...
            <div>
                <h1 class="heading">Dear ${email.split('@')[0]},<span>your order is successfully placed</span>
                    ${list_html}
                </h1>
    ...
    .then(data => {
        transporter.sendMail(mailOption, (err, info) => {
            if(err){
                res.json({'alert' : 'Error please try again'})
            }else{
                res.json({'alert' : 'Your order has been placed'}); //---> attention, the string must be equal in checkout.js <---
            }
        })
